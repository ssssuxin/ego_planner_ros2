...
namespace ego_planner
{
  ...
  {
    nh->declare_parameter<double>("optimization/lambda_smooth",-1.0);
    nh->declare_parameter<double>("optimization/lambda_collision",-1.0);
    nh->declare_parameter<double>("optimization/lambda_feasibility",-1.0);
    nh->declare_parameter<double>("optimization/lambda_fitness",-1.0);
    nh->declare_parameter<double>("optimization/dist0",-1.0);
    nh->declare_parameter<double>("optimization/swarm_clearance",-1.0);
    nh->declare_parameter<double>("optimization/max_vel",-1.0);
    nh->declare_parameter<double>("optimization/max_acc",-1.0);
    nh->declare_parameter<int>("optimization/order",3);

    nh->get_parameter("optimization/lambda_smooth",lambda1_);
    nh->get_parameter("optimization/lambda_collision",lambda2_);
    nh->get_parameter("optimization/lambda_feasibility",lambda3_);
    nh->get_parameter("optimization/lambda_fitness",lambda4_);
    nh->get_parameter("optimization/dist0",dist0_);
    nh->get_parameter("optimization/swarm_clearance",swarm_clearance_);
      ...
      exit_multi_loop2:;
        ...
        for (int j = occ_start_id; j <= occ_end_id; j++)
        {
          ...
          if (RichInfoSegs[i].first.base_point[j].size() != 1)
          {
            ...
            RCLCPP_ERROR(rclcpp::get_logger("BsplineOptimizer"), "Wrong number of base_points!!! Should not be happen!.");
            cout << setprecision(5);
            cout << "cps_" << endl;
            ...
            for (int temp_i = 0; temp_i < cps_.size; temp_i++)
            {
              if (cps_.base_point[temp_i].size() > 1 && cps_.base_point[temp_i].size() < 1000)
              {
                ...
                RCLCPP_ERROR(rclcpp::get_logger("BsplineOptimizer"), "Should not happen!!!");
          ...
          if (grid_map_->getInflateOccupancy(base_pt_reverse)) // Search outward.
          {
            ...
            if (l > l_upbound)
            {
              ...
              RCLCPP_WARN(rclcpp::get_logger("BsplineOptimizer"), "Can't find the new base points at the opposite within the threshold. i=%d, j=%d", i, j);
              segments.erase(segments.begin() + i);
              RichInfoSegs.erase(RichInfoSegs.begin() + i);
              seg_upbound--;
              i--;
          ...
          else
          {
            ...
            RCLCPP_WARN(rclcpp::get_logger("BsplineOptimizer"), "base_point and control point are too close!");
      ...
      else if (RichInfoSegs[i].first.size == 1)
      {
        ...
        if (grid_map_->getInflateOccupancy(base_pt_reverse)) // Search outward.
        {
          ...
          if (l > l_upbound)
          {
            ...
            RCLCPP_WARN(rclcpp::get_logger("BsplineOptimizer"), "Can't find the new base points at the opposite within the threshold, 2. i=%d", i);
            segments.erase(segments.begin() + i);
            RichInfoSegs.erase(RichInfoSegs.begin() + i);
            seg_upbound--;
            i--;
        ...
        else
        {
          ...
          RCLCPP_WARN(rclcpp::get_logger("BsplineOptimizer"), "base_point and control point are too close!, 2");
    ...
    for (int i = 0; i < max_traj_nums; i++)
    {
      ...
      while (digit_id < seg_upbound && selection[digit_id] >= VARIS)
      {
        ...
        if (digit_id >= seg_upbound)
        {
          ...
          RCLCPP_WARN(rclcpp::get_logger("BsplineOptimizer"), "Should not happen!!! digit_id=%d, seg_upbound=%d", digit_id, seg_upbound);
      ...
      while (/*seg_id < RichInfoSegs.size() ||*/ cp_id < cps_.size)
      {
        ...
        else
        {
          ...
          RCLCPP_ERROR(rclcpp::get_logger("BsplineOptimizer"), "Shold not happen!!!!, cp_id=%d, seg_id=%d, segments.front().first=%d, segments.back().second=%d, segments[seg_id].first=%d, segments[seg_id].second=%d",
                    cp_id, seg_id, segments.front().first, segments.back().second, segments[seg_id].first, segments[seg_id].second);
  ...
  std::vector<std::pair<int, int>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
    ...
    for (size_t i = 0; i < segment_ids.size(); ++i)
    {
      ...
      else
      {
        ...
        RCLCPP_ERROR(rclcpp::get_logger("BsplineOptimizer"), "a star error, force return!");
        vector<std::pair<int, int>> blank_ret;
        return blank_ret;
      }
    }
  ...
  void BsplineOptimizer::calcSwarmCost(const Eigen::MatrixXd &q, double &cost, Eigen::MatrixXd &gradient)
  {
    ...
    double t_now = _clock.now().seconds();
    ...
    for (int i = order_ - 1; i <= i_end; ++i)
    {
      ...
      if (occ)
      {
        ...
        if (j < 0) // fail to get the obs free point
        {
          ...
          RCLCPP_ERROR(rclcpp::get_logger("BsplineOptimizer"), "ERROR! the drone is in obstacle. This should not happen.");
        ...
        if (j >= cps_.size) // fail to get the obs free point
        {
          ...
          RCLCPP_WARN(rclcpp::get_logger("BsplineOptimizer"), "WARN! terminal point of the current trajectory is in obstacle, skip this planning.");
    ...
    if (flag_new_obs_valid)
    {
      ...
      for (size_t i = 0; i < segment_ids.size(); ++i)
      {
        ...
        else
        {
          ...
          RCLCPP_ERROR(rclcpp::get_logger("BsplineOptimizer"), "a star error");
      ...
      for (size_t i = 0; i < segment_ids.size(); ++i)
      {
        ...
        if (got_intersection_id >= 0)
        {
          ...
          RCLCPP_WARN(rclcpp::get_logger("BsplineOptimizer"), "Failed to generate direction. It doesn't matter.");
  ...
  bool BsplineOptimizer::rebound_optimize(double &final_cost)
  {
    ...
    rclcpp::Time t0 = _clock.now(), t1, t2;
        ...
        for (double t = tm; t < tmp * 2 / 3; t += t_step) // Only check the closest 2/3 partition of the whole trajectory.
        {
          ...
          if (flag_occ)
          {
            ...
            if (t <= bspline_interval_) // First 3 control points in obstacles!
            {
              ...
              RCLCPP_WARN(rclcpp::get_logger("BsplineOptimizer"), "First 3 control points in obstacles! return false, t=%f", t);
              return false;
            }
...
//#define USE_SECOND_CLEARENCE_CHECK
      ...
      else
      {
        ...
        RCLCPP_WARN(rclcpp::get_logger("BsplineOptimizer"), "Solver error. Return = %d, %s. Skip this planning.", result, lbfgs::lbfgs_strerror(result));
  ...
  bool BsplineOptimizer::refine_optimize()
  {
    ...
    do
    {
      ...
      else
      {
        ...
        RCLCPP_ERROR(rclcpp::get_logger("BsplineOptimizer"), "Solver error in refining!, return = %d, %s", result, lbfgs::lbfgs_strerror(result));
      }
